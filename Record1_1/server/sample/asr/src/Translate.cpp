...
static void set_config(bds::BDSSDKMessage &cfg_params) {
    ...
    const std::string app_id = "11163651";
    const std::string app_key = "dG8bOMVe8cCeTUZ11EfQ48Uu";
    const std::string app_secret = "ACzWCZO3x8AsHZNZ1sZxhrtxoOcwqXSY";
...
/**识别单个文件*/
static std::string recog_one_file(string );
...
std::string Translate(const char *filename) {
    ...
    std::string result = recog_one_file(filename); // 识别一个文件
 ...
 * 识别一个文件
 */
static std::string recog_one_file(string filename) {
    ...
    asrdemo::AsrdemoController controller("TestName", "../../../resources/asr_resource/", listener);
    _config(controller);
