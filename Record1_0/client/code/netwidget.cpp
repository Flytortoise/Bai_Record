...
void NetWidget::CreateInit()
{
    ...
    PortLineEdit = new QLineEdit(tr("9999"));
...
void NetWidget::RecvMessage()
{
    ...
    if(json_error.error == QJsonParseError::NoError)
    {
        if(parse_document.isObject())
        {
            ...
            if(obj.contains("merged_res"))
            {

                tmpValue = obj.take("merged_res");
                if(tmpValue.isObject())
                    tmpObject = tmpValue.toObject();
                else
                    return;

                if(tmpObject.contains("semantic_form"))
                {
                    tmpValue = tmpObject.take("semantic_form");
                    if(tmpValue.isObject())
                        tmpObject = tmpValue.toObject();
                    else
                        return;

                    if(tmpObject.contains("parsed_text"))
                    {
                        tmpValue = tmpObject.take("parsed_text");
                        //if(tmpValue.isString())
                            parsed_text = tmpValue.toString();
                    }

                    if(tmpObject.contains("raw_text"))
                    {
                        tmpValue = tmpObject.take("raw_text");
                       // if(tmpValue.isString())
                            raw_text = tmpValue.toString();
                    }
