...
char audio_dir[256] = "./test.wav"; // 与THREAD_NUM一起决定测试的文件名
...
void asr_set_config_params(bds::BDSSDKMessage &cfg_params) {
    ...
    const std::string app_id = "11072846";
    const std::string chunk_key = "ypcdAcc30au3DVp6oVXgXqmZ";
    const std::string secret_key = "5pf9hMb7GywX2dMuIG9gPhe55F7MAycG ";
...
void asr_output_callback(bds::BDSSDKMessage &message, void *user_arg) {
    ...
    switch (status) {
        ...
        case bds::EVoiceRecognitionClientWorkStatusChunkNlu: { //语义解析
            const char *buf;
            result_size = 0;
            message.get_parameter(bds::DATA_CHUNK, buf, result_size);
           // fprintf(result_output_file, "[%s]nlu result:", time);
            if(result_buff != NULL)
                delete []result_buff;
            result_buff = new char[result_size + 1]();
...
int Translate(const char *filename)
{
    ...
    for (int i = 0; i < THREAD_NUM; ++i) {
        ...
        snprintf(file_names[i], 256, "./%s", filename);
